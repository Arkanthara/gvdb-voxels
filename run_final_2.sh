...
echo "Timestamp,CPU_Usage(%),Memory_Usage(%),RAM_Usage(%),GPU_Usage(%),GPU_Memory_Usage(%)" >$output_file
...
while kill -0 $pid 2>/dev/null; do
  ...
  echo "$(date +'%Y-%m-%d %H:%M:%S'),$cpu_usage,$mem_usage,$ram_usage,$gpu_usage_val,$gpu_mem_usage" >>$output_file
  sleep 1
done
