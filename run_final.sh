...
echo "Timestamp,CPU_Usage(%),Memory_Usage(%),GPU_Usage(%),GPU_Memory_Usage(%)" >$output_file
...
while kill -0 $pid 2>/dev/null; do
  cpu=$(get_cpu_mem_usage)
  gpu=$(get_gpu_usage)
  cpu_usage=$(echo $cpu | awk '{print $1}')
  mem_usage=$(echo $cpu | awk '{print $2}')
  gpu_usage=$(echo $gpu | awk -F ', ' '{print $1}')
  gpu_mem_usage=$(echo $gpu | awk -F ', ' '{print $2}')
  echo "$(date +'%Y-%m-%d %H:%M:%S'),$cpu_usage,$mem_usage,$gpu_usage,$gpu_mem_usage" >>$output_file
  sleep 1
done
